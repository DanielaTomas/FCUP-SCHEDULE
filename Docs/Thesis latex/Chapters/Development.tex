...
\begin{algorithmic}[1]
\Procedure{BEST\_CHILD}{children, c\_param = 1.4}
...
\Function{EXPANSION}{}
	...
	\State Retrieve available periods and corresponding valid rooms 
\\
...
To improve the effectiveness of timetable generation, we also implemented a diving strategy in\ac{mcts} (\ref{Algorithm \ref{selection_with_diving} and \ref{diving_algorithm}. This diving strategy enables the algorithm to follow and deepen promising paths, aiming to improve the convergence speed and solution quality.
...
\begin{algorithmic}[1]
\Procedure{DIVE}{}
		\State selected\_node $\gets$ Pop next node 
		\State simulation\_path $\gets$ Pop simulation path
		...
		\Else
			\State Set current node to selected\_node
               		\State \Return True
            	\EndIf
            	 \State Remove this entry from the queue
            \EndIf
\EndProcedure
\end{algorithmic}
\end{algorithm}

\subsection{Example of the Diving Approach}
